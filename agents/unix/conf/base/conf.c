...
ta_unix_conf_base_init(void)
{
    ...
    /* Remaining legacy subtree: the interface collection (its own
     * brother chain is empty, see the node_interface declaration). */
    if ((rc = rcf_pch_add_node("/agent", &node_interface)) != 0)
        return rc;
